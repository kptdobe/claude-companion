...
enum WindowActivator {
    ...
    /// PaperclipAI runs Claude headless (`sdk-cli`) but each session lives in a
    /// browser tab. Switching to the *exact* issue tab isn't reliable — Chrome's
    /// AppleScript only exposes one profile (often not the user's active one),
    /// and Chrome ignores the scripted active-tab setter visually — so we simply
    /// bring Chrome to the front. Non-Paperclip sdk-cli sessions surface Claude.
    private static func activateSDK(for session: Session) {
        if session.issueKey != nil {
